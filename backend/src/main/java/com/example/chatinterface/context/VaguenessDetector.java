...
/**
 * Detecte si une question utilisateur est trop vague et necessite une clarification.
 * Utilise un prompt de juge structure avec exemples few-shot et score de confiance.
 * Fail-open : toute exception retourne CONTINUE (on ne bloque jamais l'utilisateur).
 */
@Component
@Order(1)
public class VaguenessDetector implements ContextStep {
    ...
    @Override
    public ContextStepResult execute(String prompt, ContextLlm llm) {
        try {
            log.info("[VAGUENESS] Analyzing prompt: '{}'", prompt);
            String rawResponse = llm.analyze(SYSTEM_PROMPT, prompt);
            ...
            return ContextStepResult.interrupt("clarification_needed", reason, suggestions, confidence);

        } catch (Exception e) {
            // Fail-open : on ne bloque jamais l'utilisateur en cas d'erreur
    ...
    /**
     * Extrait le JSON d'une reponse LLM qui peut contenir des markdown code fences.
     */
    private String extractJson(String raw) {
        if (raw == null) return "{}";
        String trimmed = raw.trim();

        // Supprime les code fences markdown (```json ... ``` ou ``` ... ```)
